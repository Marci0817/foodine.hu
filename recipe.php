<?php
    ...
    if (!isset($_SESSION["username"])) {
        header("location: recipe_utmutat.php");
    }
    if(isset($_POST["recept_feltolt"])){
        $recept_sessionname = base64_encode($_SESSION["username"]);
        $recept_szerzo_show = base64_encode($_POST["recept_szerzo_show"]);
        $recept_nev = base64_encode($_POST["recept_nev"]);
        $recept_db_keszit = $_POST["recept_db_keszit"];
        $recept_db_adag = $_POST["recept_db_adag"];
        $recept_hozvalok = base64_encode($_POST["recept_hozvalok"]);
        $recept_menete = base64_encode($_POST["recept_menete"]);
        $recept_date = date('Y-m-d H:i:s');
        $recept_kategoria = base64_encode($_POST["recept_kategoria"]);
        $recept_egyeb = base64_encode($_POST["recept_egyeb"]);

        //Account ID lekérdezés
        $sql = "SELECT id,pont FROM users WHERE username = ?";

        $stmt = $db->prepare($sql);
        $stmt->bind_param("s", $_SESSION["username"]);
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($account_id,$points_user);
        $stmt->fetch();
        $stmt->close();
        ...
        $sql = "INSERT INTO recept (account_id, img, name, szerzo, receptnev, db_keszit, db_adag, hozvalok, menete, date, kategoria, egyeb) 
        VALUES ('$account_id','$img_name' ,'$recept_sessionname', '$recept_szerzo_show', '$recept_nev', '$recept_db_keszit', '$recept_db_adag', '$recept_hozvalok', '$recept_menete', '$recept_date', '$recept_kategoria', '$recept_egyeb')";

        if ($db->query($sql) === TRUE) {}

        //Pont hozzáadás, és feltöltés

        $points_user += 5; //$points_user += 3;
        $pont_upload = "UPDATE users SET pont='$points_user' WHERE id='$account_id'";
        if ($db->query($pont_upload) === TRUE) {}
...
<html>
    ...
    <body>
        ...
        <div class="fbox1">
            <h3>Recept feltöltés</h3>
            <hr>
            <form action="" method="post" enctype="multipart/form-data">
                <div class="fbox2">
                    <label>Feltöltő neve</label><br>
                    <input type="text" name="recept_szerzo_show" value="<?php echo $_SESSION['username'];?>" readonly>
                    <small class="form-text text-muted">Nem lehet módosítani</small>
                </div>
                <div class="fbox2">
                    <label>Recept neve</label><br>
                    <input type="text" name="recept_nev" placeholder="Pl.: Mákosguba" required>
                </div>
                <div class="fbox2">
                    <label>Elkészítési idő (percben)</label><br>
                    <input type="number" min='1' name="recept_db_keszit" placeholder="Pl.: 3 perc" required>
                </div>
                <div class="fbox2">
                    <label>Hány főre jut?</label><br>
                    <input type="number" min='1' name="recept_db_adag" placeholder="Pl.: 4" required>
                </div>
                <div class="fbox2">
                    <label>Kérlek írd le a hozzávalókat!</label><br>
                    <textarea class="form-control" name="recept_hozvalok" rows="3" placeholder="Külön sorba a hozzávalókat" required></textarea>
                    <small class="form-text text-muted">Az átláthatóság érdekébe a hozzávalókat külön sorba írd.</small>
                </div>
                <div class="fbox2">
                    <label>Kérlek írd le az elkészítés menetét!</label><br>
                    <textarea class="form-control" name="recept_menete" rows="3" required></textarea>
                    <small class="form-text text-muted">Ha teheted kérlek használj számozást</small>
                </div>
                <div class="fbox2">
                    <label>Fotó feltöltése az ételről</label><br>
                    <input type="file" class="form-control-file" name="fileToUpload" id="fileToUpload" required>
                    <small class="form-text text-muted">Kötelező, de elég egy netről letöltött kép is. :) ( Ajánlani tudom igényes, ingyenes fotokért a <a href="http://pixabay.com/hu/images/search/food" target="_blank">Pixabay</a>-t )</small>
                </div>
                <div class="fbox2">
                    <label for="validationDefault04">Kategória</label><br>    
                    <div class="col-md-3 mb-3">                
                        <select class="custom-select" name="recept_kategoria" id="validationDefault04" required>
                            <option selected disabled value="">Válassz...</option>
                            <option value='Húsételek'>Húsételek</option>
                            <option value='Édes sütik'>Édes sütik</option>
                            ...
                            <option value='Sós süti'>Sós süti</option>
                            <option value='Saláta'>Saláta</option>
                            <option value='Szósz'>Szósz</option>
                            <option value='Sós krémek'>Sós krémek</option>
                            <option value='Édes krémek'>Édes krémek</option>
                            <option value='Köret'>Köret</option>
                            <option value='Befőtt'>Befőtt</option>
                            <option value='Halételek'>Halételek</option>
                            <option value='Szendvicsek'>Szendvicsek</option>
                            <option value='Lekvárok-dzsemek'>Lekvárok-dzsemek</option>
                            <option value='Gluténmentes'>Gluténmentes</option>
                            <option value='Egyéb'>Egyéb</option>
                        </select>
                        <small class="form-text text-muted">Ha nincs ilyen kategória, akkor vagy az egyébet válasszd vagy vedd fel velünk a kapcsolatot.</small>
                    </div>
                </div>
                <div class="fbox2">
                    <label>Egyéb</label><br>
                    <textarea class="form-control" name="recept_egyeb" rows="2" ></textarea>
                    <small class="form-text text-muted">Ide tippeket is írhatsz.</small>
                </div>
                <input type="submit" class="btn btn-outline-success" name="recept_feltolt" value="Feltöltés"/> <del>+3 pont</del> <span class="badge badge-secondary" style="background-color:green;">+5 pont</span> ( KIVÉTELES IDŐSZAK eddig: 2020.08.01 00:00 <a href="pointlist.php" target="_blank">részletek </a>) </h5>
            </form>
        </div>
