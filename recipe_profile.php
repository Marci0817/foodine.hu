<?php
    ...
    $get_info = "SELECT username, date_join, pont, img_profile FROM users WHERE id = ?";

    $stmt = $db->prepare($get_info);
    ...
    $stmt->execute();
    $stmt->bind_result($user_recept_count);
    $stmt->fetch();
    $stmt->close();
...
<html>
    ...
    <body>
        ...
        <div id="profile_tart">
            ...
            <h3><a href="profile.php?id=<?php echo $_GET['id']; ?>"><span style="text-decoration: underline;"><?php echo $profile_name;?></span></a> receptjei</h3>
            <hr>
            <?php 
                if($user_recept_count == 0){
                    $alert_ki = "<div class='alert alert-warning' role='alert'>$profile_name-nak/nek még nincsennek receptjei.</div>";
                    echo $alert_ki;
                }
            ?>
            ...
            <?php 
                $user_id = $_GET["id"];
                $query = "SELECT * FROM recept WHERE account_id='$user_id' ORDER BY id DESC";  
            ...
            <!-- Statisztika -->
            <hr>
            <h3>Legutóbb regisztrált felhasználók</h3>
            <div class="card-deck">
            <?php 
