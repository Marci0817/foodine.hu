...
<html>
    <head>
        ...
        <link rel='stylesheet' type='text/css' media='screen' href='css/recipe_details.css'>
    ...
    <body>
        ...
        <?php  
            ...
            $query = "SELECT * FROM recept WHERE id='$receptID'";  
                    ...
                    <div id="carouselExampleSlidesOnly" class="carousel slide" data-ride="carousel">
                        <div class="carousel-inner">
                            <div class="carousel-item active">
                                <img src="uploads/<?php echo $row['img'];?>" class="d-block w-100" alt="...">
                            </div>
                        </div>
                        <?php 
                                $szerzo_name = base64_decode($row['szerzo']);

                                $sql = "SELECT id FROM users WHERE username = ?";
                                $stmt = $db->prepare($sql);
                                $stmt->bind_param("s", $szerzo_name);
                                $stmt->execute();
                                $stmt->store_result();
                                $stmt->bind_result($szerzo_id);
                                $stmt->fetch();
                                $stmt->close();
                            ?>
                        <small class="form-text text-muted">Feltöltve: <?php echo $row['date'];?></small>
                        <h1><?php echo base64_decode($row['receptnev']); ?></h1>
                        <p id="keszit">Feltöltötte: <a href="profile.php?id=<?php echo $szerzo_id?>" ><?php echo base64_decode($row['szerzo']); ?></a></p>
                        <p>Elkészítési idő: <?php echo $row['db_keszit']; ?> perc, Adag: <?php echo $row['db_adag']; ?> fő</p>
                        <hr>
                        <div class='tartalom_recept'>
                            <p class="tartalom_tagolas">Hozzávalók</p>
                            <pre><?php echo base64_decode($row['hozvalok']);?></pre>
                            <br>
                            <p class="tartalom_tagolas">Elkészítés</p>
                            <?php echo base64_decode($row['menete']);?>
                            <br>
                            <p class="tartalom_tagolas">Egyéb</p>
                            <pre><?php echo base64_decode($row['egyeb']);?></pre>
                        </div>
                    </div>
